...
export const ADJUDICATED: Readonly<Record<string, readonly Adjudication[]>> = {
  ...
  'find-steed': [
    ...
    {
      clause: 'does **not** take its turn immediately after theirs',
      why: 'a-turn-a-spell-inserts-into-the-order',
      note: 'SRD: "In combat, it shares your Initiative count", and "If you have the Incapacitated condition, the steed takes its turn immediately after yours". The first sentence is built — `resolveSummonEffect` reads the summoner’s rung off the order and seats the steed on it — and the second is a **position** rather than a number. `byInitiative` ranks a count and then a tiebreak, and `Combatant.tiebreak` states in its own docstring why the engine may not manufacture the second: the book leaves ties to the GM, so the engine takes that decision as an input rather than inventing one. Seating the steed one below its summoner was tried and reverted for exactly that: every combatant’s tiebreak is 0 unless a DM states one, so a creature on the same count would still come between the two, and the invented number would have bought nothing while breaking the rule it stood on.',
    },
    {
