...
| **`an-area-a-slot-scales`, with three consumers** — Confusion, Fog Cloud and Create or Destroy Water | conformance | the foreman’s GREEN decision of 2026-09-14, reasoning in the same architecture record. Fog Cloud is recorded as blocked on nothing today and is corrected on arrival; Confusion’s clause is filed `'table'` under protest until this lands. **Creation is deliberately excluded** — its Cube is the matter created, not a region of effect, and one consumer is below the two-writer bar |
| **The stat block’s payload is parsed by nothing**, and `a-stat-block-created-mid-fight` cannot be *built* until something reads it | mechanism | the debt Fable’s decision accepts explicitly, so it is recorded rather than discovered. The monster parser already reads this identical structure at `###` (`monsters.ts:15–20`), so the shape’s brief names that parser. **Do not add a `statBlock` field before then** — it would be a member nothing consumes |
| **A creature joining a fight mid-round cannot use a `moment === 'initiative'` feature**, because `commands/features.ts:446` gates those on `turnsTaken > 0` | mechanism | IE-055’s builder and reviewer, raised as a note rather than a defect. Pre-existing semantics about the first turn of a fight, and it only became reachable when `joinCombat` existed. Surprise and anything about *when* a fight starts are the DM’s, so the question is narrow: does a latecomer’s arrival count as their initiative moment? |
| **The architecture extraction multiplied the surface a change can falsify, and it showed up the next day.** IE-053 renamed one field and made assertions false in **four** `docs/design/` documents; two of its four review rounds were stale-design-doc sweeps | docs | the foreman, ruling on IE-053’s round count. Before 2026-09-14 that prose was in one file and a builder updated one file. **This is a real cost of the refactor, not an argument against it** — the prose is more findable and more owned than it was — but a task touching a cross-cutting field should now expect a design-doc sweep as part of the work rather than as a review finding, and the brief should say which documents to check. Worth a line in `qb-builder.md` or in the brief template |
...
| Two tranche-6 SRD quotations close early on sentences the book continues — IE-038 quotes "On a turn, you can expend only one spell slot." where `spells.md:167` goes on "to cast a spell", and the Concentration sentence likewise | conformance | IE-052’s guard reports both. **It is the only purchase a containment test has on the elision class**, which the brief classifies as not catchable — so the instrument reaches one step further than it claims |
| Two stale `file:line` references in closed briefs: `IE-003` names `commands.ts:6764` against a file the split left at 206 lines, `IE-021` names `scripts/coverage.ts:626` against one of 231 | conformance | IE-052. The live briefs carry no `path:line` reference at all today, so this is historical only |
...
| 2026-09-14 | merge (tranche authority, **three deviations ratified**) | IE-052 | merged `d35edd6`, 13/13, reviewer PASS at high confidence at **round 4** — three rounds of `DEFECTS`, a foreman ruling, then a bounded confirmation pass. **The builder stopped rather than ship a required behaviour it could not honestly meet, and the reviewer classified the remainder as a brief-compliance question rather than escalating it as architecture. Both were right.** Requirement 6 is ratified unmet: the guard is a report, not a suite gate. **Condition 12 fired for the second time this tranche** — the branch passed its own gauntlet and the combination gave 3 failed / 8,537, all three fixtures that were true when written, because IE-050 created `fold/casting.ts` and IE-051 moved the Acid Arrow docstring; the repairs now derive their facts and each asserts its own premise. **The guard found what a human had not**: `/qb` read as a path was a real scan defect, and it appeared in a brief between review rounds because the foreman had put it there. The report was brought to **0 findings over 7 live briefs** at a cost of four restructured paragraphs — and one of those four hid from a `grep` by spanning a line break while the guard, which normalises whitespace, found it at once. One foreman edit to builder code, declared: a prose count in a docstring that the same docstring forbids twenty-five lines earlier, raised by the reviewer after the PASS |
| 2026-09-14 | merge (tranche authority, **one process deviation ratified**) | IE-053 | merged `5556e3d`, 13/13, reviewer PASS at high confidence. **The headline is not the rename.** `ongoing-compatibility.ts` keyed its catalogue fill on `version !== ONGOING_RECORD_VERSION`, so *any* bump would have routed every version-2 record through the fill and overwritten an area pinned at the cast with the book as it reads now — the exact hazard pinning the area was for, one constant edit away, and in the builder’s own words *invisible to both frozen logs because both are pre-versioned and take the fill either way*. A wrong number with no symptom, found as a precondition and driven test-first: the mutation bumped the version without the re-keying and watched a pinned 15-foot Cube become the catalogue’s 20. **Fable’s IE-047 merge condition honoured exactly** — the derived-`on` gate’s third case is flipped, not deleted. Foreman verification went to the fixtures rather than the digest: `golden-log-2.json` carries ten `spell-ongoing` records with **no `version` field at all**, so the oldest upgrade path is pinned by an immutable log and 11,425 checkpoints walk it. **Four review rounds against a three-round cap**, judged GREEN by the builder and reported rather than buried: ratified on the merits — no round raised an architectural finding and findings shrank monotonically — but the extension is the foreman’s to grant, and `qb-builder.md` is reworded so stopping at three reads as the success it is. **And two of those four rounds were stale-design-doc sweeps**: this change falsified prose in four `docs/design/` documents, which is a cost of the 2026-09-14 extraction arriving one day later |
| 2026-09-14 | merge (re-scoped, tranche authority) + **YELLOW to Fable** | IE-056 | merged `e435867`. Builder `ARCHITECTURE_BLOCKED`, reviewer `ESCALATE` **with no defects** and the explicit line *"it is safe to merge as it stands and the escalation is about the two decisions, not about the code"* — the IE-047 shape, handled the same way. **Three of four families moved off zero** and are now briefable from a read count; the fourth reads 0 honestly. **The builder took the zero and pinned why instead of bending a guard.** The escalation: `sentenceGaps` demands an adjudication for every marker-bearing sentence while `unanchoredPhrases` forbids a phrase not occurring exactly once — and the SRD prints summon stat blocks as HTML tables with **three identical `<td>SAVE</td>` cells in one row**, which the foreman confirmed in `packages/srd/raw/spells.md` before spending Fable on it. No reading satisfies both guards. Find Steed, Giant Insect and Summon Dragon are the entire unread list of the book’s highest-ranked family, and every escape redefines what "read" means for all sixty read entries and ~145 grandfathered ones — which is why it is Fable’s and not the foreman’s. Travelling with it: no shape id names **an area a slot scales**, so Confusion’s clause is filed `table` under protest with a test pinning that it is wrong on purpose, and Fog Cloud is a second consumer currently recorded as blocked on nothing |
| 2026-09-14 | merge (tranche authority) | IE-055 | merged `4f23895`, 13/13, reviewer PASS at high confidence on the **first** round with **no deviations** — the second clean first-round pass of the tranche. Closes the hole IE-046 found: a `not_in_combat` request that had to name two commands because neither did the job. **The comparator extraction was verified by the foreman as exactly behaviour-preserving** — the three-term inline sort becomes `byInitiative(a, b) || a.index - b.index` with `byInitiative` the first two terms — and the test that makes it mean something is an oracle asserting `addCombatant` and `startCombat` agree over 21 initiative/tiebreak combinations, the only test that catches a second comparator diverging on a tie. The owner’s boundary held: nothing here decides *when* a fight begins, and the initiative number is the caller’s exactly as at `beginCombat`. **`joinCombat` is deliberately unguarded and says why against `relocateCreature`’s precedent** — the second task this tranche to reason from it, with IE-057 live on the same precedent; a third would be a pattern worth naming in the design document. It also corrected a comment in `refusals.test.ts` that had become a false claim — "there is no `addCombatant`, so the order only ever shrinks" — and widened that sweep rather than just fixing the prose |
| 2026-09-14 | **YELLOW answered** | IE-056 | **Fable rejected all four options the escalation offered, including the one the builder recommended and the foreman leaned toward — and the finding underneath them is better than the question that was asked.** Neither conformance guard gives way: **the corpus is wrong**. `parse/spells.ts:52` bounds a spell entry at `####` only, and the three `##` stat-block headings are the only `##`/`###` headings after `## Spell Descriptions`, so each is swept into the preceding spell’s `higherLevel` — while the stat block’s own `#### Traits/Actions` are silently dropped. The foreman measured the generated corpus to confirm it: `find-steed` carries **1,191 characters** of stat block filed as upcast text, `giant-insect` 1,149, `summon-dragon` 1,221, each after the one real sentence. **Nobody decided that; it is a parser seam**, and `coverage-data.ts:70` and `spell-tracking.test.ts:369` consume it too. The fix is two seams — bound at any heading level, and count `unanchoredPhrases` over *distinct* printed units, which preserves the rule’s stated purpose exactly because identical text has no neighbouring licence to take. **The escalation’s third question is answered provably**: no read entry prints a marker-tripping duplicate, and not by luck — such a duplicate makes an entry unreadable, so the sixty are structurally untouched. **And the problem was wider than the five escalated**: `control-weather`, `reincarnate` and `scrying` are caught by the same defect and pinned by nothing. Recorded in `docs/architecture/read-corpus-and-repeated-units-2026-09-14.md`; **tranche 8’s work, not tranche 7’s** — Fable’s own line is "Stays inside the approved brief: NO" |
