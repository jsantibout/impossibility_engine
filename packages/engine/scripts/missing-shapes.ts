...
export const MISSING_SHAPES = {
  ...
  'a-long-casting-time':
    'CLAUDE.md: "In combat the obligation is a state machine on the caster’s own turns, and it is built." **The mechanism is whole.** IE-034 built the clock half — a casting of a minute or more is declared, runs on the clock and settles, and a Ritual is cast the same way — and IE-041 built the obligation: `continueCasting` spends the Magic action SRD asks for on each of the caster’s turns, and a turn that ends without it fails the rite derived, through `releaseCasting`, with the slot never spent. **IE-036 then read the twelve paragraphs this was the only blocker for and wrote all twelve as tracked definitions**, so what it now blocks is the forty-two spells that name it *and something else*; not one of them is blocked on this shape alone, which is why the second number below is zero rather than the shape being retired. Those forty-two are **not** re-filed: each is blocked by a shape this map already names, and re-reading them belongs with whichever task is briefed from the shape they are waiting on.',
