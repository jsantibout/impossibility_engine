...
describe('toolSchemas', () => {
  ...
  it('publishes exactly the surface it publishes, pinned', () => {
    ...
    expect(toolSchemas(player())).toHaveLength(80);
    expect(toolSchemas(dm())).toHaveLength(98);
    expect(JSON.stringify(toolSchemas(player())).length).toBe(110482);
    expect(JSON.stringify(toolSchemas(dm())).length).toBe(132789);
  });
});
