...
/**
 * **Releasing a casting on one creature moved to `ongoing-spells.test.ts`.**
 *
 * `endSpellEffectOn` found its casting through `caster.concentration` and
 * wrote `condition-removed` events directly; `endOngoingSpell` names the
 * casting by id and writes the `spell-ended` the reducer already had, so its
 * fixture needs an ongoing record — which `castSpell` alone does not write and
 * `resolveSpell` does. Every one of the cases that stood here is converted
 * there, beside the record they are now about.
 */
