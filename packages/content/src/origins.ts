  ...
  features: [
    {
      ...
      level: 1,
      automation: 'engine',
      note: 'Executed. SRD "You gain Heroic Inspiration whenever you finish a Long Rest" is a pool of one the rest refills, and the glossary sentence, "you can expend it to reroll any die immediately after rolling it, and you must use the new roll", is read two ways because it has two halves. A failed ability check or saving throw opens the test-rolled window Indomitable already answers, and the reroll is taken there, costing no Reaction. Every other die a command of the roller’s own throws is an election: the holder states, on that command, the condition they would spend the use under — reroll if this misses, reroll if this fails, reroll if the die shows this or lower — and the engine reads it against the die it threw, so a weapon attack roll, one of that swing’s own damage dice, and a check or save that succeeded are all reached with no window anybody has to settle. A window on every die was tried and withdrawn on 2026-09-23, because a table would then settle one before every next roll; the election was ruled for on 2026-09-24. The pool is spent only where the condition was met, the first face stays on the record, and the new roll is used whatever it shows. What the election does not yet reach is every die thrown by a command the holder did not send: a spell attack roll and a spell’s damage dice, a saving throw a spell or a turn boundary owes, Initiative, a death save, and the damage of a swing that was held for a Reaction — each is thrown inside a settlement the roller did not send a command for, so there is nowhere to state the condition. The one of those a caller can ask for is the held swing’s damage, and asking is refused rather than quietly ignored.',
      grants: {
        kind: 'reaction',
        costsReaction: false,
        reach: { kind: 'self' },
