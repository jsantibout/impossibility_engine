...
const UNASSERTED_ON_PURPOSE: Readonly<Record<string, string>> = {
  nothing_to_interrupt:
    'a defensive re-read, reachable from nothing. `interrupt-casting` is written by exactly one definition — Counterspell — and `triggerRefusal` refuses `no_trigger` before a slot or a Reaction is spent when no casting is open. The one path that skips that check is a **readied** spell, whose trigger was declared and paid for when it was readied; SRD requires a readied spell to have a casting time of an action, and Counterspell is a Reaction, so it cannot take that path. Between the check and the re-read only the Counterspell’s own events are folded in, and its `spell-cast` allocates a fresh casting id rather than settling the open one, so `pendingCasting` still stands. Re-verified for the **second** way a casting can be open: a casting of a minute or more is concentrated on from its declaration, and `releaseCasting` now drops the pending record with that Concentration — but Counterspell takes no Concentration and is not a long casting itself, so its own batch writes no `concentration-ended` at all and the rite’s caster keeps theirs. `long-casting.test.ts` drives a Counterspell at a Ritual and watches it resolve, which is the re-read reaching the casting; both of the older halves are asserted in `refusals.test.ts`. This branch is what stays correct if any of the three ever stops being true.',
};
