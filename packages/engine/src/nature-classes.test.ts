...
import { BARD, BARDIC_DIE } from './bard.js';
import { CIRCLE_OF_THE_LAND, DRUID, WILD_SHAPE_USES } from './druid.js';
import { FAVORED_ENEMY_USES, RANGER } from './ranger.js';
import { PALADIN } from './paladin.js';
import { allClasses, createCharacter, planCharacter, type CharacterChoices } from './creation.js';
...
const plan = (choices: CharacterChoices) => unwrap(planCharacter(choices), choices.classId);

const built = (choices: CharacterChoices, who: string): GameState =>
  fold('seed', unwrap(createCharacter(choices, id(who)), 'create') as GameEvent[]);
...
describe('the Ranger', () => {
  ...
  it('folds, survives JSON, and replays prefix by prefix', () => {
    const events = unwrap(createCharacter(ranger(), id('sorrel')), 'create');
...
describe('all twelve classes are in', () => {
  it('registers every class the SRD publishes', () => {
    expect(allClasses().map((c) => c.name).sort()).toEqual([
      'Barbarian',
      'Bard',
      'Cleric',
      'Druid',
  ...
  it('covers every spellcasting shape the SRD has', () => {
    const styles = allClasses().map((c) => c.spellcasting?.style ?? 'none');
  ...
  it('has both full and half casters, and classes with and without cantrips', () => {
    const casters = allClasses().filter((c) => c.spellcasting !== undefined);
