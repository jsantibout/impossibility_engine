...
const UNASSERTED_ON_PURPOSE: Readonly<Record<string, string>> = {
  no_casting:
    'a defensive re-read of events this same function has just produced, reachable from nothing. `keptRunning` needs the id of the casting `resolveCastWith` allocated a line above it, and it reads that id off the `spell-cast` in the returned batch rather than deriving it a second time — two derivations of one number is how a record comes to name a casting nobody made. `castSpellWith` writes that event on every path but one: a *declared* casting, which writes `spell-declared` instead and is reached only through `CastCommand.hold`, a field this caller does not set and could not sensibly set — a smite cast on a hit has no window to be interrupted in. So the branch is what stays correct if that ever changes, and the alternative to it is a non-null assertion on an event nobody checked. The surrounding behaviour is driven end to end by `burning-smite.test.ts`, which watches the record and the timer the id is written into.',
