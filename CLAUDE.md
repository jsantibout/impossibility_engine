...
8. **Numbers live in `COVERAGE.md` and `LEDGER.md`**, regenerated by
   `npm run coverage` and `npm run ledger`. `COVERAGE.md` is diffed in CI;
   both go stale loudly, under a freshness test inside `npm test`. Do not put
   a count in prose.
...
npm run ledger                            # regenerates LEDGER.md: what is left to reach level 5
```

Before pushing: all five green. **`COVERAGE.md` and `LEDGER.md` are the
coordinator's to commit, once, after a batch merges** — a builder runs
`npm run coverage` to read the number its brief names, reports it, and then
`git checkout -- COVERAGE.md`. Six
...
`STATUS.md` says what runs, what does not and the rulings that stand, capped
at two thousand words and holding no "next". **`docs/ROADMAP.md` is what
happens next**: the road to playable levels 1–5 through Infinite Realms, the
plan a foreman works from. `LEDGER.md` is how far along it we are, generated
by `npm run ledger` and stale-checked like `COVERAGE.md`. `CONTRIBUTING.md` says how two people share the tree. `docs/dev/WORKFLOW.md` is the agent workflow.
