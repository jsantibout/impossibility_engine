...
describe('a paladin made from choices radiates a real aura', () => {
  it('has the feature on the sheet, with the SRD’s own radius', () => {
    ...
    expect(standing.map((e) => e.feature)).toEqual([
      ...
      'defense',
      // The last is not an aura and is not on until the feature is switched
      // on: the light SRD Sacred Weapon's weapon emits is a standing effect
      // requiring the feature to be active, which is what every `whileActive`
      // grant becomes. Its other two sentences are **not** here and used to be
      // — the Charisma bonus and the damage type are keyed to the one weapon
      // the use imbues now, which is a `GrantedWeaponRider` rather than
