...
import { CLERIC, LIFE_DOMAIN, SRD_CONTENT } from '@ie/content';
import { classCasting, type SpellcastingState } from '@ie/engine';
import { asCharacterId, isErr, expect as unwrap } from '@ie/shared';
import { armorClass } from '@ie/engine';
...
import { createCharacter, advanceCharacter, planCharacter, type CharacterChoices } from '@ie/engine';
...
const made = (over: Partial<CharacterChoices> = {}): GameEvent[] =>
  unwrap(createCharacter(SRD_CONTENT,cleric(over), BRANNOR), 'create');
...
const rejects = (over: Partial<CharacterChoices>, code: string): void => {
  const result = planCharacter(SRD_CONTENT,cleric(over));
...
describe('a Cleric prepares from the class list, not from a book', () => {
  it('is made without a spellbook at all', () => {
    const plan = unwrap(planCharacter(SRD_CONTENT,cleric()), 'plan');
...
describe('a Cleric levels up the same way a Wizard does', () => {
  it('gains the level 4 slots and keeps what it was wearing', () => {
    ...
    const events = unwrap(
      advanceCharacter(before, SRD_CONTENT, BRANNOR, {
    ...
    expect(after.creatures.brannor!.equipped.map((held) => held.id)).toEqual(['chain-shirt', 'shield']);
  });
  ...
  it('advances without ever mentioning a spellbook', () => {
    const log = made();
    const events = unwrap(
      advanceCharacter(fold('seed', log), SRD_CONTENT, BRANNOR, {
        cantrips: ['sacred-flame', 'guidance', 'light', 'mending'],
        preparedSpells: [
          'inflict-wounds',
          'healing-word',
...
describe('the Wizard is unchanged by any of this', () => {
  ...
  it('still refuses a Wizard who prepares a spell not in their book', () => {
    ...
    const result = planCharacter(SRD_CONTENT,wizardish);
    expect(isErr(result)).toBe(true);
...
describe('both classes are reachable by id', () => {
  it.each([['cleric'], ['wizard']])('finds %s', (classId: string) => {
    const plan = planCharacter(SRD_CONTENT,{ ...cleric(), classId, level: 1 } as CharacterChoices);
