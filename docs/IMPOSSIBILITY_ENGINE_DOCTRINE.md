...
| 3 | `roll-recorded` carries the number rolled, and `scenario.test.ts` replays a four-round fight byte-for-byte from the log alone; a second seed folds it identically. **A debt, recorded rather than claimed away:** a damage event carries the *total*, and the individual faces reach the log only where a reaction window opened one. The owner ruled on 2026-09-21 that each die must reach it — Sorcerous Burst explodes on an 8 and Chromatic Orb reacts to two dice showing alike, and a Great Weapon Fighting substitution nobody can read is a rule a player has to take on trust. |
...
| 7 | The engine throws every die a model can reach: `rollD20Recorded` is the only d20 an AI-held surface can cause, only `rolls.ts` stamps `engine`, and `checkExternalSource` refuses any caller claiming that stamp. The external-roll functions are swept by name out of both AI-held surfaces, in every import form, by `boundary.test.ts` and `dm/boundary.test.ts`. **What this row said until 2026-09-21** — "damage takes a `RollId` the engine issued" — was true of the outcome and false of the mechanism: no damage command takes one, and what held the invariant was reachability. Said plainly: a model cannot reach a function that accepts a number, so a model cannot supply one. |
