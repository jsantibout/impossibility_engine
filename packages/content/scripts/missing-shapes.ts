  ...
  slow: [
    {
      ...
      note: 'the other half of the −2, and the half with no reader. The penalty to Armour Class is written — a `bonus` rider aimed at `ac` with the sign turned round — and this one cannot be, because a rider carries a `BonusApplies` list and nothing narrower: aimed at `save` it would land on every saving throw the target ever makes, including the Wisdom save this spell itself calls for. The ongoing side of the narrowing axis exists and the rider does not carry it.',
    },
    {
    ...
    {
      clause: 'repeats the save at the end of each of its turns',
      why: 'a-repeat-save-with-no-condition-to-hang-it-on',
  ...
  'animal-messenger': [
    {
      ...
      why: 'a-target-rule-the-format-cannot-state',
      note: 'the Challenge Rating this save turns on **is held now** — `CreatureState.cr`, pinned at the arrival off the block, which `save.autoSucceedIf.challengeRatingAbove` reads — and the Tiny that picks the Beast was built a batch before it, so this spell writes neither of the facts the shape above is still missing. **What blocks it is where the verdict goes.** Its failure buys an errand across the countryside, which is the DM’s from the first word to the last, so the die decides nothing a rule can hold but its own answer; `verdict_before_the_record` refuses `recordsOutcome` in a casting’s own effect list, because that list resolves before the record a verdict would be written onto exists. Whether a save whose whole content is its verdict may be written there — published by the casting that rolled it, in its own outcomes — is a decision nobody has taken, and this spell is the one asking for it',
    },
  ],
  polymorph: [
    {
