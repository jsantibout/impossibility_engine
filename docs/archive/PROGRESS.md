...
| Every event type reachable | The other nine DM-declared facts get their commands, so **all 91 declared event types are emitted by engine code** — seventeen had no producer when the number was first derived. The sweep asserting it closes the class rather than the instances | `9847661` |
| Outcome riders | Leaf riders in fixed slots on the three kinds that produce an outcome, with the branch fixed by the host and a rider that never rolls — Fable's design, built. And `on` gets one rule: a Range: Self casting no longer discards whom it caught, so Dispel Magic finds Sunbeam's victim | `35ad5e3` |
| A guard for zero-user members | Every member of the definition format must have a user or a written exemption — four had neither. The special-case sweep now reads every source file, proven per file rather than asserted | `b1a3b3c` |
| Scene commands | Eight commands for the scene-setup family, so a tool surface can start an encounter at all — proved by a scenario that builds one from nothing through commands only, with no hand-written event | `6d3cb92` |
...
| A feature definition is validated data | `checkFeatureDefinition` over all ~230 class, subclass and species features — the third audit's open item, and the structural answer to nine features that declared `automation: 'engine'` on the strength of a note claiming a pool none of them declared. Rule 4 derives the readable grant kinds from the readers rather than from a list, which is what stops the guard becoming the hand-kept claim it exists to catch | `43fe3d0` |
| One enumerator for four sourced grants | `bonuses`, `armorClasses`, `rollModifiers` and `grantedDefenses` were walked by hand in five places, and a fifth family added to three of them is how a grant comes to be released by a dispel and not by a deadline. The family list is now **derived from `CreatureState`'s own shape**, so a fifth makes the enumerator a compile error naming the property it lacks — a stronger guard than the test the brief asked for | `7e7717b` |
| `resolveEffects` split per kind | 1,008 lines to 214, thirteen resolvers over one shared context, and **not one behaviour change** — every resolver body equal to its original branch line for line, verified by reversing the transformations and diffing, twice, independently. Paid down before five more effect kinds land on it | `71bf406` |
| Every context request names a command | Seventeen `satisfyWith` strings told a tool surface to append an **event**, which is the one thing the layer above the engine must never do — every fact has had a command since IE-012 and IE-016. A derived sweep over the barrel keeps it that way, and the one genuine exception carries an exemption whose claim is checked three ways. Six review rounds, every one passing the implementation and returning prose | `dd97c84` |
| A citation is held against the file it names | The guard asked whether a description *mentioned* `CLAUDE.md`, not whether `CLAUDE.md` said what it claimed — so two misquotes shipped past it in one tranche. It resolves by naming now, through an alias table, and found five more on its first run: three of them counts quoted from a `PROGRESS.md` that had since been re-derived | `c0d4ca9` |
| Characters nobody can see are refused | A stray `U+0008` in the validator's regex meant the audit flag matched nothing, so the one signal the manual-audit design leaves the foreman was invisible to every fresh session from the day it was written. A C0 sweep over the tracked set now refuses them, `check-queue.mjs` has a self-test driving the real CLI, and **the guard caught the task that built it** — the tool that wrote its source decoded the escapes into six real control characters, invisible while untracked, named the instant they were staged | `024c8fd` |
| Every refusal reaches a caller under its own code | Two call sites rewrote the action economy's refusals to `not_enough_movement`, so moving out of turn was reported under a code that contradicted its own reason string; a blank pool key emitted an event whose fold threw instead of returning a value. Both fixed, the dead sites removed or exempted on facts a test sweeps, and a code nothing in the engine returned is gone | `74e3c46` |
| The validator judges untyped input | Every branch of the spell-definition validator reports rather than throws, and `effectLists` is the one enumeration of a definition's effect lists — which **removed** a second copy in `checkGrantLifetimes`. Fable's option (a), taken after three rounds established the question was not the builder's to answer; it also closed a live hole, the rider-is-a-leaf denylist having reached one list of three | `d53ae63` |
| `COVERAGE.md` has one writer | The report was regenerated at import time by two tests, so the gauntlet's `git diff --exit-code COVERAGE.md` asserted that the suite had run rather than that the committed file was right — every merge record claiming it byte-clean was true for the wrong reason. The measurement moved to a side-effect-free module, the write sits under a main-module guard, and an AST sweep holds every script in the directory to it | `ff15526` |
| A casting states which creatures it is fighting | SRD gives five spells' saves Advantage "if you or your allies are fighting **it**" — per target, and both Charms can be upcast, so the boolean the brief prescribed was silently wrong for one of two targets. Stated as a list of ids, `unaffected`'s existing shape, with the one asymmetry that an empty list is an answer and is never elided. **And the derived blocker map turned out to be wrong about Enthrall**: it prints an automatic success, not Advantage | `d185bd6` |
| Speed is read live, through one reader | Three spellings of a creature's Speed, none able to see a feature grant, and a turn budget storing a **remainder seeded at the pinned Speed** — so nothing could raise an allowance and the fold measured the command's own event against a different number. `speedOf` is the one reader; the budget stores what was spent and what a Dash gained. It found a wrong number already shipped: a Fighter with Exhaustion 3 who Dashed had 45 feet where the SRD prints 30, asserted against its own comment | `9887829` |
| A casting ended by a trigger | Eight executed spells carried a printed sentence the engine could not honour — Invisibility ending when its target attacks, Mage Armor when armour goes on, Charm Person when you damage the target. A closed `endsEarly` list pinned at the cast, ended by a **derived pass** the way expiry and a lost Concentration already are, with termination made structural after a mutation hung the fold. Partial spells 52 → 47, derived | `db62430` |
| A Speed an effect changes | A fifth sourced grant, read through IE-031's one reader and ended through the door the other four use — Longstrider executed, Ray of Frost's reduction expiring on the **first production `grants` timer** the engine has ever created, Hypnotic Pattern's Speed 0 riding its Charmed save. The bundle that said no Speed shape finished anything splits: the modifier retires, `movement-modes` stays as the honest remainder | `fcf40cc` |
| A long casting time, and rituals | The largest blocker in the book — 54 spells touched, 12 finishable, and it was in none of the three rankings the repository kept before the blockers were derived. Outside combat a casting of a minute or more is a **declared** casting that completes on the clock, concentrating on itself from declaration so the damage save already fires; a Ritual adds its ten minutes and expends nothing. In combat the refusal stands and names the per-turn obligation still missing | `c3fc27e` |
| A rider on later attacks, and a duration the slot changes | Divine Favor and Hunter's Mark execute: a sixth sourced grant hangs typed extra damage on the caster's later attacks, and a per-definition band turns a slot level into seconds — per definition because Hex and Hunter's Mark print **different tables**, and one shared field would have made one of them silently wrong. Mass Suggestion finished as a side effect | `fbc7e31` |
| Teleportation inside the scene | A position change that spends no movement, provokes nothing, and is refused rather than guessed — Misty Step executed, Dimension Door and Tree Stride defined. The fixture that proves the scene check exists is **a 40-foot room with a 30-foot spell**: in the 600-foot hall the range check answers first and the scene check hides behind it for ever. And `sceneFor` has one home at last, on its third copy | `062441c` |
| The end of the current turn | A fifth `Duration` member, and the one the SRD writes that no anchor names: "until the end of the current turn" is a moment in the order rather than a fact about a creature, so it takes **no `of`** and reads whoever is taking the turn at resolution. `endOfNextTurn` said of the creature whose turn it is resolves **two** turn-endings away, a full round late, which is why three consumers could not be written before it. Stinking Cloud ships as its writer. It also repaired the guard meant to catch a member with no user: `RiderDuration` is the format's one union mixing literals with an object arm, the sweep required the *whole* right-hand side to be literals, and **the type its own docstring names as the example was the one type it could not see** | `e0639e4` |
| Pending castings keyed by casting id | The single `pendingCasting` slot was an accident of its first user — a Counterspell window open for an instant — and it refused **every other creature's** casting for the whole of a ten-minute rite. The owner rejected the per-caster invariant that was proposed to replace it, and the SRD agrees: Longer Casting Times puts the Magic action obligation on the caster's **own** turns and expends no slot until completion, Concentration breaks only on a spell that *requires* it, and a Reaction is taken on somebody else's turn — so a wizard mid-rite may legally hold a second casting open. **Casting identity, not caster identity**, is the record's key; no uniqueness rule survives anywhere, and what refuses a second casting is the action economy, the turn's one slot, or Concentration's single door. `answeredCasting` never picks between candidates: an omitted id resolves only where there is nothing to choose between. The two nesting refusals stay and now say in the string a caller receives that they are this engine's limit rather than the book's | `673178b` |
| A spell’s unbuilt blockers, anchored to the sentences it prints | `BLOCKED_ON` was a bare list of shape ids anchored to nothing, so an entry naming one blocker for a spell with three passed every guard — and **every wrong prediction this map has made** (Mind Blank, Protection from Energy, Enthrall, Magic Weapon, True Strike) was an omission of exactly that kind, never a wrong entry. Entries are now clause-anchored records quoting a phrase from the spell’s own parsed paragraph, matched exactly once in both directions, with a per-sentence guard over the honesty guard’s markers; `consumersOf` reports **two** `finishes` numbers, read and unread, and a tranche may be planned from the first. No new hand list and no shape id invented — 83 before, 83 after, identical sets. On its first day it reported that the twelve spells the next task was briefed to define are all **unread** | `5605992` |
| The coverage report stops contradicting itself | A regex classifier filed every spell under "the hardest thing its text needs" and printed a hand-written blocker column beside the **derived** table — 43 spells under a casting time of a minute or more against the derived 54, with a different disposition, in one file. The three-documents-three-answers failure arriving inside one report, and tranche 6 was briefed from that report. Deleted; `PARTIAL_SPELLS` is now derived from the adjudication map and came out equal to the hand list it replaced, 48 for 48. `VERIFIED_SPELLS` stays written out, with a docstring saying why it is not symmetric: it is a claim about which tests drive which spell, and nothing derivable says that | `606c142` |
| The fold split along its own seams | `events.ts` was 5,432 lines and it serialised tranche 5 — seven of ten mechanism tasks touched it, and that chain **was** the critical path. Now `state.ts` holds the state root, `events.ts` is the union and a barrel at 1,180 lines, and a `fold/` directory holds the release doors, the area detectors, the turn raising, expiry, the end triggers and `applyOne`. The oracle was byte-identity, not the suite: all 88 declarations code-identical with comments removed, independently re-derived by the reviewer, and a committed graph script that reports **ACYCLIC**. **It bought less concurrency than predicted**, and honestly so: `applyOne` stayed one switch, so every reducer-case task still meets in one file | `3190f91` |
| Twelve spells a casting time alone blocked | The largest blocker in the book, spent: Alarm, Clairvoyance, Commune with Nature, Fabricate, Find the Path, Hallucinatory Terrain, Identify, Illusory Script, Instant Summons, Legend Lore, Magic Mouth and Mending are all cast for real — the action, the Concentration, the clock, the settlement by id — with what they *do* left to the table. Tracked 45 → 57; the shape falls from 54 blocks and 12 finishes to 42 and 0. **Alarm is the definition that made an existing rule checkable**: all ten previously tagged spells print “Action or Ritual”, so the ten minutes a Ritual adds could not be told from a constant by any test; Alarm prints “1 minute or Ritual” and comes to 660. And two of the twelve print a dismissal clause the honesty guard’s markers cannot see, which is an edge of an instrument that reads English, found by use | `fe42caf` |
| A readied spell states the facts a casting states | `ReadiedResponse` carried a spell id, a casting id and a level and nothing else, so the three Dominates could not be readied — and **not by being refused**: the Ready succeeded and spent the action and the slot, and every release came back `fought_fact_required` until the hold’s own deadline lifted it. A wedged hold that consumed resources. The four facts ride on the record now, validated through the **same** `declaredFacts` the atomic cast and the declaration use and applied through the same normaliser — proved by mutation rather than by inspection: emptying that validator reddens the readied path and both older doors together. Absent means absent, so both frozen logs fold unchanged | `12adff7` |
| A casting of a minute or more runs in combat too | IE-034 built the clock half and left the per-turn obligation named as missing; this is it. `continueCasting` spends the Magic action SRD asks for on each of the caster’s turns, and a turn that ends without it **fails the rite derived** — no event, through `releaseCasting`, the slot never spent, exactly as a lost Concentration. `resolveTurn` stops blocking on a rite and still blocks on an instant window. The recorded wedge — a rite open when a fight begins — is driven end to end and gone. **And deleting the in-combat refusal exposed what it had been hiding**: a turn-anchored Duration could never reach the declaration branch while long castings were refused in combat, and in combat it resolves — pinning a deadline a minute before the spell exists, the bug `lastsSeconds` was built against. Refused now, before the slot | `c8b6ae1` |
| A casting is ended by its id | `endSpellEffectOn` found its casting through **whoever was concentrating on it** — the last place in the command layer a casting was addressed by its holder — so it could not end a non-Concentration ongoing spell at all. `endOngoingSpell(castingId, on)` replaces it, `resolveCast` is demoted from the barrel, and the mutation that proves the point is restoring the holder lookup: seven tests redden, every non-Concentration case. **And reading the SRD paragraph rather than the brief’s quotation of it found a fourth guard**: the book names three duration forms and prints the free dismissal under **Time Span** alone, so an Until-dispelled casting — Continual Flame, with its 50 GP of ruby dust — has no caster-side ending at all. Bought for the correctness of the addressing: three spells still blocked, none finished | `f10cf01` |
| A monster enters through a command, with its printed immunities | `adaptMonster` produced condition immunities and `conditionApplicability` answered three ways — and **nothing outside `monster.ts` called either**, so a Zombie was Poisoned like anybody and nothing above the engine could add a monster without folding an event itself. `addCreature` wraps the adapter, `conditionImmunitiesOf` is the one gatherer beside `defensesOf`, and `applyConditionTo` reads it. The three-valued answer survives by **where each entry goes**: unconditional into state, qualified into `unverified`, nothing invented — so the Vampire Familiar’s “Charmed (except from its vampire master)” still reaches a person to adjudicate. An immune target inside a casting is skipped rather than aborting it, because refusing would advance the generator and emit nothing | `beda2e7` |
| A turn-anchored moment outside combat asks for a turn order | The owner’s decision: a rule such as Ray of Frost’s “until the start of your next turn” needs an actual turn timeline, and outside combat there is none — so the engine **neither invents six seconds nor invents a fight**. It asks. A seventh `ContextRequest.kind` carries the printed clause and names `beginCombat`; a creature outside a *running* order gets a different request naming `rollInitiativeFor` as well. `resolveDuration` is untouched, because a pure helper returns the bare refusal and the command that knows which rule wanted the fact attaches the request. **The sweep is what earned it**: the brief named three call sites, source held six, and three converted for free because `schedule` is a single door | `09a52de` |
| Whether `OngoingSpell.on` can be derived — measured, and answered no | The audit recommended deriving it on **869 checkpoints, 0 mismatches**. The gate ordered before the removal reproduced that number exactly and then falsified the premise: a **tracked** spell owns nothing on its target ever, so the rule answers *nobody* and Darkvision stops being dispellable, while a target the casting was **released on** forbids the cast-time seed that would rescue it. The fixture was blind rather than the measurement wrong — `golden-log-2.json` holds no tracked spell cast at a target — and what found it was wiring the comparison into `applyEvent` so the whole suite checked it. Fable’s answer: `on` is two facts of different provenance, store what only the cast knows and derive what the world holds; briefed for a later tranche. What landed is the measurement, a **linear** frozen-log prefix test, and fixtures for the three mutations that had survived the entire suite | `62cb640` |
