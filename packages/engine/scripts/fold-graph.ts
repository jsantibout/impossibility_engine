 ...
 * would not be a precondition. It prints the list it read, so the population is
 * in the output rather than in this comment.
 *
 ...
 * contribute no value edges at all; the seven `fold/` seams are where the
 * question is. `fold/index.ts` declares nothing — it re-exports — so it is not
 * read and not in the layout.
 */
 ...
 * both. `fold/index.ts` is absent because it declares nothing.
 */
const SOURCES = [
  'events.ts',
  'state.ts',
