...
import { armorClass, proficiencyBonusForLevel, skillModifier } from './character.js';
import { fold, type GameEvent, type GameState } from './events.js';
import { carrying } from './commands.js';
import { ROGUE, SNEAK_ATTACK_DICE, THIEF } from './rogue.js';
...
const made = (over: Partial<CharacterChoices> = {}): GameEvent[] =>
  unwrap(createCharacter(rogue(over), NYX), 'create');
...
const rejects = (over: Partial<CharacterChoices>, code: string): void => {
  const result = planCharacter(rogue(over));
...
describe('Expertise is the same grant the Wizard has', () => {
  ...
  it('doubles the proficiency bonus on the chosen skills', () => {
    const plan = unwrap(planCharacter(rogue()), 'plan');
