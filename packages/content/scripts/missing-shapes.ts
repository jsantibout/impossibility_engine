...
export const MISSING_SHAPES = {
  ...
  'a-casting-that-casts-another-spell':
    '`docs/design/casting.md`, on the interrupted casting: "It is not a general interruption framework — there is **no stack**, and a Counterspell answering a Counterspell is refused rather than nested." A spell that casts another as part of itself, or duplicates one of a lower level, needs exactly the stack that was declined. Several castings may be open at once now, keyed by casting id — but that is several *independent* castings rather than one nested inside another, and the two relationship rules that refuse the nesting still stand. **Storing one to go off later turned out not to need it** (W7-S21): SRD Glyph of Warding’s spell glyph casts the stored spell at the inscription — its slot spent, nothing left open — and pins the request on the glyph’s record (`OngoingSpell.stored`), which `triggerGlyph` lets go through the path a readied spell is released by. What is left under this name is the nesting: Wish-style duplication, and a casting that runs another inside its own resolution.',
  ...
  'a-second-place-to-put-a-creature':
    'the engine holds one scene and three kinds of elsewhere — ethereal, extradimensional, inside another creature — each with its way back pinned; a plane with its own geometry, a demiplane the party walks into, or a distant copy of a creature still has nowhere to be. `docs/design/spell-definitions.md`: "A destination *outside* the scene is different in kind ... there is one scene, so Plane Shift and Word of Recall have no position to move anybody to", and `docs/design/casting.md`: "the real fix is the doctrine’s multiple-scenes seam".',
  falling:
...
export const ADJUDICATED: Readonly<Record<string, readonly Adjudication[]>> = {
  ...
  'find-familiar': [
    {
      clause: 'the special senses printed on the familiar’s own stat block',
      why: 'senses-beyond-declared-sight',
      note: 'SRD: "you can see through the familiar’s eyes and hear what it hears until the start of your next turn, gaining the benefits of any special senses it has." **The borrowing is built** (W7-S21): `borrowSenses` is the Bonus Action, the permission is `KeptSummons.lends`, and `canSee` answers yes where the familiar sees while `sensesOf` lends the caster every sense the engine holds for it. What does not arrive is the block’s own Senses line — an Owl’s Darkvision — because `adaptMonster` puts a block’s senses nowhere a sheet reads, so a familiar has only the senses a casting gave it. That is the bestiary’s gap rather than this spell’s, and it closes here the day a block’s Senses line reaches `sensesOf`.',
    },
    {
