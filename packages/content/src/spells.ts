...
export const FIND_STEED: SpellDefinition = {
  ...
  unmodelled: [
    ...
    'the steed shares its rider’s Initiative count and its turn is not **pinned** immediately after theirs when the rider has the Incapacitated condition: a joiner is seated after everyone it exactly ties with, so today it does land there — and it lands behind any third creature the DM has put on that count, because a rung is a number and settling the tie with one of the engine’s own would invent a decision the SRD leaves to the GM',
