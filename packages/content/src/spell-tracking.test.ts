...
describe('every spell this batch added is cast for real', () => {
   ...
   *
   * Two went the same way. `ActionRule` was derived from four SRD sentences
   * and no definition wrote one; those two write three of them — Wind Walk's
   ...
   *
   * **Aid is the third, and it left through the same door for a shape of its
   * own.** Its whole text is twenty-two words about a hit point maximum, so a
   * pass that could not move one had nothing to write and tracked it; the
   * `hit-point-maximum` effect is that writer, and the spell is now executed
   * end to end with no sentence left over — which is why it is also the first
   * of these to leave `ADJUDICATED` entirely rather than move a clause into
   * it.
   */
  const EXECUTED_SINCE: readonly string[] = ['aid', 'magic-jar', 'wind-walk'];
