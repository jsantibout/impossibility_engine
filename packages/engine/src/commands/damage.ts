...
import { actionRulesOn, type CastingDamageFeature, defensesOf, sheetAsItStands } from '../standing.js';
...
const NOTHING_GAINED: DropSpoils = { events: [], unverified: [] };
 ...
 *
 * **Both damage roads call it, and so does the sentence that drops a creature
 * without damage.** "Reduce an enemy to 0 Hit Points" is what a sword, a Fire
 * Bolt and a Sea Hag's glare each do, and a reader wired into one of them
 * would be a rule that stops working when the hag shows up. It is
 * `standingReductionOf`'s shape: one function, called wherever the moment
 * happens.
 *
...
export function dealSpellDamage(
  ...
  readonly concentration: ConcentrationConsequence;
  /**
   * What a feature watching this blow could not settle — a side nobody has
   * declared, a holder nobody has placed. Empty for every blow that dropped
   * nobody, which is almost all of them.
   */
  readonly unverified: readonly string[];
}> {
  ...
  // **What the blow bought whoever was watching, after it landed.** SRD Dark
  // One's Blessing asks about the *outcome* rather than the swing, so the
  // question is put to the world the damage has already changed — and only
  // where this blow is the one that took the creature to 0, which
  // `droppedToZeroBy` reads off the hit points that are actually there.
  const landed = resolved.value.events.reduce(applyEvent, state);
  const spoils = droppedToZeroBy(state, landed, target)
    ? rewardsForDropping(landed, target, options.by ?? null)
    : NOTHING_GAINED;
  ...
  return ok({
    ...
    // spoils come last, because nothing is owed until the creature is down.
    events: [
      ...(dice === null ? [] : [dice]),
      ...warded.value.events,
      ...wardCounted,
