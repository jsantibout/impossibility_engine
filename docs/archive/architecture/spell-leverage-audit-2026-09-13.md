...
> ## Superseded, and kept as a record — 2026-09-13, IE-015
>
> **Every per-shape count in this document is superseded by
> `packages/engine/scripts/missing-shapes.ts`, and the figures below are left
> exactly as they were written.** A historical record corrected in place stops
> being a record of what was believed, and what was believed is the evidence
> the fourth whole-engine audit needed: this document said a granted
> Resistance had **4** consumers, `PROGRESS.md`'s ranked map said **17**, and
> the SRD text supports **2 whole and 1 partial**. Three documents, three
> numbers, none of them derived.
>
> The sentence under "Why this audit was cheap" — *"So the numbers here are
> derived, not estimated"* — is the claim that did not hold. IE-004's
> vocabulary was machine-readable and IE-002's reading was thorough, but the
> counts in sections C, D, F and G were still taken by hand from prose, and
> three of the shape ids they were taken from turned out to be **bundles**.
>
> Two of the four remaining candidates did not survive re-derivation. **C2**,
> "a selector for the save a casting forces", makes **zero** consumers whole:
> five of its six spells want a *declared fact* ("you or your allies are
> fighting it") rather than a selector axis, and the sixth wants a creature
> type. **C1** was overstated roughly twofold — Rage's resistance is already
> executed, and Absorb Elements is not in SRD 5.2.1 at all.
>
> For any count, ask the map: `consumersOf(shape)` answers *how many spells
> does this block, and which*, and reports **`blocks`** and **`unblocks`**
> separately — every spell a shape touches, and the spells it is the only
> blocker for. `COVERAGE.md` prints both for every shape, regenerated by
> `npm run coverage`. What this document is still good for is the *argument*:
> which primitives compose, which storage patterns already have three users,
> and why a fourth is a transcription rather than a new idea.
