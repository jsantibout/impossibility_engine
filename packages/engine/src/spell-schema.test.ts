...
const FORMAT_EXEMPTIONS: Readonly<Record<string, string>> = {
  ...
  'SpellEffect[save].verdictOnly?':
    'SRD Animal Messenger prints exactly this sentence — "must succeed on a Charisma saving throw, or it attempts to deliver a message for you" — and the definition is not written here because it needs a **second** field this branch does not have: the automatic success for a beast whose Challenge Rating is not 0, which `autoSucceedIf.challengeRatingAbove` carries and which arrived on `main` with the cast-line bestiary track after this one branched. Writing the definition without it would transcribe a spell that catches every beast in the book, which is the confident wrong answer rather than the missing one. The reader is live and driven: `verdict-only-save.test.ts` casts a homebrew definition carrying the mark, the die is rolled, the answer reaches the caller as `SpellTargetOutcome.save` and no ongoing record is written. The day Animal Messenger is written this fails rather than going on excusing a field that now has a writer.',
  'SpellCheck.dc?':
