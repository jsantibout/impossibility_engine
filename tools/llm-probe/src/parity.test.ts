...
const AUDIT: readonly {
  ...
  },
  {
    ...
    fates: {
      ...
      usingLine: because(
        'the printed line a move is made on — SRD Giant Seahorse\'s Bubble Dash, SRD Troll\'s Charge — which the stated-line doors handed the turn as a grant under the line\'s own source. `using_grant`\'s sibling for a stat block\'s sentence rather than a feature\'s, and `packages/tools` publishes it as `using_line`; it is withheld here for the same reason, that neither benchmark character is a creature whose block prints such a line, so a published field would be refused every value it could be sent today',
      ),
