...
import { OATH_OF_DEVOTION, PALADIN, SRD_CONTENT, WIZARD } from '@ie/content';
...
const made = (over: Partial<CharacterChoices> = {}): GameEvent[] =>
  unwrap(createCharacter(SRD_CONTENT,paladin(over), AELRIC), 'create');
...
const rejects = (over: Partial<CharacterChoices>, code: string): void => {
  const result = planCharacter(SRD_CONTENT,paladin(over));
